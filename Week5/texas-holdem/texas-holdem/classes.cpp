...
bool Comp::IsHitting() const
{
    return (GetTotal() <= 16);
}

void Comp::FlipFirstCard()
{
    if (!(m_Cards.empty()))
    {
        m_Cards[0]->Flip();
...
void Game::Play()
{
    ...
    else
    {
            ...
            ++pPlayer)
        {
            if (!(pPlayer->IsBusted()))
            {
                if (pPlayer->GetTotal() > m_Comp.GetTotal())
                {
                    pPlayer->Win();
                }
                else if (pPlayer->GetTotal() < m_Comp.GetTotal())
                {
                    pPlayer->Lose();
                }
                else
